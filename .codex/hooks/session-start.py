...
def _is_codex_inline(trellis_dir: Path) -> bool:
    try:
        from common.task_gates import is_codex_inline  # type: ignore[import-not-found]
    except Exception:
        return False
    return is_codex_inline(trellis_dir.parent)
...
def _get_task_status(trellis_dir: Path, hook_input: dict) -> str:
    ...
    gate_warning = ""
    if _is_codex_inline(trellis_dir):
        gate_warning = (
            "\nCodex inline gates: Stop hook must be enabled and approved via `/hooks` for Gate 5 completion blocking; archive/finish-work still hard-block through task.py."
        )
