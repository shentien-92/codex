...
## Marketplace Skill Index

Trellis marketplace skills live under `skills/<skill-id>/` and are indexed from `index.json`. Use these entries when helping users find, install, or customize built-in complex skills:

| Skill | Marketplace path | Category |
| --- | --- | --- |
| `trellis-meta` | `skills/trellis-meta/` | Trellis architecture, platform files, and customization reference. |
| `grill-trellis` | `skills/grill-trellis/` | Plan stress-testing against `.trellis/spec/`, glossary, and ADR context. |
| `trellis-deep-review` | `skills/trellis-deep-review/` | Optional strict evidence-based completion review gate. |
| `trellis-spec-bootstarp` | `skills/trellis-spec-bootstarp/` | Project `.trellis/spec/` bootstrapping and refresh workflow. |

## Marketplace Workflow Index

Trellis marketplace workflows live under `workflows/<workflow-id>/workflow.md` and are indexed from `index.json`. Use these entries when helping users choose or customize a workflow template:

| Workflow | Marketplace path | Category |
| --- | --- | --- |
| `native` | `workflows/native/workflow.md` | Default Trellis Plan / Execute / Finish workflow. |
| `tdd` | `workflows/tdd/workflow.md` | Test-driven Trellis workflow variant. |
| `channel-driven-subagent-dispatch` | `workflows/channel-driven-subagent-dispatch/workflow.md` | Channel worker dispatch variant for implement/check work. |
| `qunhe-native` | `workflows/qunhe-native/workflow.md` | Qunhe internal delivery workflow variant. |
